...
def load_big_dataset(categoría):
  direccion="C:/Users/pablo.garcia/Desktop/Fake news/Base de datos/BaseDatosTrain.xlsx"
  fake_news= pd.read_excel(direccion)
  cat=fake_news[fake_news['Topic']==categoría]
  cat["Text"]= cat['Text'].apply(lambda x : normalize(x) )
  cat["Text"]= cat['Text'].apply(lambda x : clean_fake(x) )


  #clean text
  cat["clean_text"]= cat["Text"].apply(lambda x : clean(x) )
...
categoría="Entretenimiento y Deporte"
df = load_big_dataset(categoría)
...
Tipos=["Accuracy RL","Accuracy NB", "Accuracy RF", "Accuracy NN", "Accuracy KNN", "Accuracy DT",
          "Accuray Bagging", "Accuracy XGB", "Accuracy SVM"]
Accuracy=[(mean(scores_RL), std(scores_RL)), (mean(scores_GNB), std(scores_GNB)),
                                              (mean(scores_RF), std(scores_RF)),
                                              (mean(scores_NN), std(scores_NN)),
                                             (mean(scores_KNN), std(scores_KNN)),
                                             (mean(scores_DT), std(scores_DT)),
                                             (mean(scores_BAG), std(scores_BAG)),
                                             (mean(scores_XGB), std(scores_XGB)),
                                             (mean(scores_SVM), std(scores_SVM))]

resultados['Modelo'] = Tipos
resultados['Acierto'] = Accuracy

resultados.to_excel('./mult_ent.xlsx')